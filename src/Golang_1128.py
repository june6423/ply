...
def p_global_assign_statement(p):
    """
    global_assign_statement : global_var_assign_statement NLD
                     | global_const_assign_statement NLD
    """
    p[0] = p[1]
    #global DEF statement Check 필요
...
def p_statement_reassign(p):
    ...
    if p[1] not in names and p[1] in global_constants:
        print(f"Error: cannot assign to global constant {p[1]}")
        return
    ...
    if p[1] in names and not isinstance(names[p[1]], type(p[3])):
        ...
        print(f"TypeError: type mismatch {global_names[p[1]]} and {p[3]}")
        return
...
def p_statement_reassign_op(p):
    ...
    if p[1] not in names and not isinstance(global_names[p[1]], type(p[3])):
        print(f"TypeError: type mismatch {global_names[p[1]]}({type(global_names[p[1]])}) and {p[3]}({type(p[3])})")
        return
